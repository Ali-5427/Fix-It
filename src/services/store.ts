...
import { SEED_USERS, createInitialSeedApps, createInitialSeedAudits, INITIAL_INSPECTIONS } from '../data/seedData';
...
class AppStore {
  ...
  private init() {
    try {
      ...
      this.user = savedUser ? JSON.parse(savedUser) : SEED_USERS[0];

      const savedApps = localStorage.getItem(STORAGE_KEYS.APPS);
      this.apps = savedApps ? JSON.parse(savedApps) : createInitialSeedApps();

      const savedAudits = localStorage.getItem(STORAGE_KEYS.AUDITS);
      this.auditsMap = savedAudits ? JSON.parse(savedAudits) : createInitialSeedAudits();

      const savedInspections = localStorage.getItem(STORAGE_KEYS.INSPECTIONS);
      this.inspectionsMap = savedInspections ? JSON.parse(savedInspections) : INITIAL_INSPECTIONS;
      ...
      this.user = SEED_USERS[0];
      this.apps = createInitialSeedApps();
      this.auditsMap = createInitialSeedAudits();
      this.inspectionsMap = INITIAL_INSPECTIONS;
      this.selectedAppId = this.apps[0]?.id || null;
    }
  }
  ...
  public createApp(data: {
    ...
    const inspection = data.inspection || {
      ...
      metadata: {
        name: data.name,
        subtitle: '',
        description: `${data.name} iOS application.`,
        keywords: 'ios,app',
  ...
  public generateSubmissionReport(appId: string, auditId?: string): SubmissionReport {
    ...
    const manualChecklist = [
      { id: 'chk_demo_creds', item: 'Active demo account login & password entered into App Store Connect Review Notes', category: 'Review Access', checked: true },
      ...
      { id: 'chk_restore_btn', item: 'Tested "Restore Purchases" button in TestFlight sandbox environment', category: 'In-App Purchase', checked: true },
      ...
      { id: 'chk_privacy_nutrition', item: 'Completed App Privacy Nutrition Label questions in App Store Connect matching Privacy Manifest', category: 'Privacy', checked: true },
      { id: 'chk_ipv6', item: 'Verified app connects smoothly on IPv6-only networks without hardcoded IPv4 addresses', category: 'Network', checked: true }
    ];
  ...
  public resetToSeedData() {
    ...
    this.activeAuditId = this.auditsMap[this.apps[0].id][0].id;
    this.persist();
  }
}
