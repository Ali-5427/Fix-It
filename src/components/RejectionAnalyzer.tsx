...
const SAMPLE_REJECTIONS = [
  {
    title: 'Guideline 2.1: Demo Account Missing',
    text: `Hello,

Guideline 2.1 - Performance - App Completeness

We were unable to review your app because we were not provided with active demo credentials. Your app requires user registration/login to access main features.

Please provide a valid username and password in the App Review Information section of App Store Connect and resubmit your app for review.`
  },
  {
    title: 'Guideline 3.1.1: In-App Purchases',
    text: `Guideline 3.1.1 - Business - Payments - In-App Purchase

We noticed that your app uses a third-party payment mechanism (Stripe) to unlock digital features and content within the app. Digital goods, subscriptions, and virtual currencies must use Apple In-App Purchase.

Next Steps:
Please remove external checkout links or migrate to StoreKit in-app purchase.`
  },
  {
    title: 'Guideline 5.1.1: Purpose String',
    text: `Guideline 5.1.1 - Legal - Privacy - Data Collection and Storage

We found that your app's Info.plist contains vague permission descriptions:
- NSCameraUsageDescription: "Camera access needed."

Your purpose string must clearly explain to users how the app uses the camera to provide core functionality.`
  },
  {
    title: 'Guideline 4.8: Sign in with Apple',
    text: `Guideline 4.8 - Design - Sign in with Apple

Your app uses Google Sign-In as a third-party login option but does not offer Sign in with Apple. Apps that offer third-party or social login services must also offer Sign in with Apple as an equivalent option.`
  }
];
...
export const RejectionAnalyzer: React.FC = () => {
  ...
  const handleAnalyze = async (textToAnalyze?: string) => {
    ...
    if (!text.trim()) {
      setErrorMsg('Please paste an Apple App Review rejection notice or select a sample.');
      return;
    }
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
        ...
        <div className="space-y-2">
          <span className="text-xs font-semibold text-slate-600">Quick-load common rejection archetypes:</span>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {SAMPLE_REJECTIONS.map((sample, idx) => (
              <button
                key={idx}
                onClick={() => handleSelectSample(sample.text)}
                className="flex items-center justify-between p-2.5 text-left rounded-xl border border-slate-200 bg-slate-50 hover:border-blue-300 hover:bg-blue-50/40 text-xs text-slate-700 transition-colors cursor-pointer group"
              >
                <span className="font-medium text-slate-800 group-hover:text-blue-700">{sample.title}</span>
                <ArrowRight className="h-3.5 w-3.5 text-slate-400 group-hover:text-blue-600 shrink-0" />
              </button>
            ))}
          </div>
        </div>
