...
export const MetadataChecker: React.FC = () => {
  const [name, setName] = useState('FitTrack Pro: Workout & Run');
  const [subtitle, setSubtitle] = useState('GPS Running & Calorie Tracker');
  const [description, setDescription] = useState('FitTrack Pro helps you log daily workouts, monitor active heart zones, and sync with Apple Health. Includes GPS run tracking and nutrition logging.');
  const [keywords, setKeywords] = useState('fitness,running,gps,calorie tracker,health,workout,intervals');
  const [promotionalText, setPromotionalText] = useState('New update includes watchOS 11 support and AI coaching insights!');
  const [privacyPolicyUrl, setPrivacyPolicyUrl] = useState('https://example.com/fittrack/privacy');
  const [supportUrl, setSupportUrl] = useState('https://example.com/fittrack/support');
  ...
  const loadBadSample = () => {
    setName('PulseChat - Secure Messenger & Audio Channels App'); // 52 chars (Limit 30)
    setSubtitle('#1 Best Free Chat App on iOS and Android'); // Competitor + Free claim + 41 chars
    setDescription('Download the official PulseChat app. Also available on Android APK and Google Play. Free group messaging.');
    setKeywords('chat, messaging, android, google play, free calls, best chat app, group messenger, audio calls, encrypted'); // > 100 chars
    setPrivacyPolicyUrl('http://insecure-site.com/privacy'); // Insecure HTTP
    setSupportUrl('http://insecure-site.com/help');
  };

  const loadGoodSample = () => {
    setName('PocketScanner AI'); // 16 chars
    setSubtitle('Fast PDF & Document Scanner'); // 27 chars
    setDescription('PocketScanner AI transforms your iPhone into a portable document scanner. Automatically detect borders, sharpen text, OCR across 30 languages, and export multi-page PDFs.');
    setKeywords('scanner,pdf,ocr,documents,receipts,contracts'); // 45 chars
    setPrivacyPolicyUrl('https://pocketscanner.app/privacy');
    setSupportUrl('https://pocketscanner.app/support');
  };
      ...
      <div className="border-b border-slate-200 pb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded-xl border border-red-200 bg-red-50 hover:bg-red-100 px-3.5 py-1.5 text-xs font-semibold text-red-700 transition-colors cursor-pointer"
          >
            Load Non-Compliant Sample
          </button>
          <button
            onClick={loadGoodSample}
            className="rounded-xl border border-emerald-200 bg-emerald-50 hover:bg-emerald-100 px-3.5 py-1.5 text-xs font-semibold text-emerald-700 transition-colors cursor-pointer"
          >
