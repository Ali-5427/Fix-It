...
import { INITIAL_INSPECTIONS } from '../data/seedData';
...
export const UploadModal: React.FC<UploadModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'file' | 'plist' | 'preset'>('file');
  ...
  const [selectedPresetKey, setSelectedPresetKey] = useState('app_fittrack');
  ...
  const handleFileSelected = (selectedFile: File) => {
    ...
    setCustomBundleId(`com.developer.${inferredName.toLowerCase().replace(/[^a-z0-9]/g, '')}`);
    setErrorMsg(null);
  };
  ...
  const processPipeline = async (inspection: NormalizedAppInspection, appName: string, bundleId: string) => {
    ...
    try {
      ...
      const app = store.createApp({
        name: appName || inspection.appName || 'My iOS App',
        bundleId: bundleId || inspection.bundleId || 'com.example.app',
        primaryCategory: customCategory || inspection.metadata.category || 'Utilities',
        currentVersion: inspection.version || '1.0.0',
        currentBuild: inspection.build || '1',
        inspection
      });
  ...
  const handleStartAnalysis = async () => {
    if (activeTab === 'file') {
      ...
      const inspection = parseInspectionData(rawPlistText, '', [], customAppName || 'Custom App');
      await processPipeline(inspection, customAppName || 'Custom App', customBundleId || 'com.example.custom');
    } else if (activeTab === 'preset') {
      const presetInspection = INITIAL_INSPECTIONS[selectedPresetKey];
      if (!presetInspection) return;
      await processPipeline(presetInspection, presetInspection.appName, presetInspection.bundleId);
    }
  };

  const loadSamplePlist = () => {
    setRawPlistText(`<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
	<key>CFBundleIdentifier</key>
	<string>com.example.photobooth</string>
    ...
    setCustomAppName('PhotoBooth Sample');
    setCustomBundleId('com.example.photobooth');
  };
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          {isProcessing ? (
            ...
            <>
              ...
              {/* TAB 3: Demo Archetypes */}
              {activeTab === 'preset' && (
                <div className="space-y-3">
                  <p className="text-xs text-slate-600">
                    Select a sample iOS project archetype to inspect real compliance preflight checks:
                  </p>
                  
                  <div className="space-y-2">
                    {[
                      { id: 'app_fittrack', name: 'FitTrack Pro (Sample)', category: 'Health & Fitness', desc: 'Sample iOS project with camera/health permissions & Privacy Manifest check.', risk: 'SAMPLE' }
                    ].map(preset => (
                      <div
                        key={preset.id}
                      ...
                      >
                        <div>
                          ...
                          <p className="text-[11px] text-slate-600 mt-0.5">{preset.desc}</p>
                        </div>
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded border text-blue-700 border-blue-200 bg-blue-50">
                          {preset.risk}
                        </span>
                      </div>
                    ))}
