...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'landing' | 'dashboard' | 'audit' | 'rejection' | 'metadata' | 'screenshots' | 'admin' | 'privacy'>('landing');
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 flex flex-col selection:bg-blue-600 selection:text-white">
      ...
      {/* Top Demo Banner for instant exploration */}
      {selectedApp?.isDemo && currentView === 'audit' && (
        <div className="bg-blue-50/90 border-b border-blue-200/80 px-4 py-2 text-xs text-center flex items-center justify-center gap-2 backdrop-blur-md">
      ...
      {currentView !== 'landing' && (
        <footer className="border-t border-slate-200 bg-slate-50/80 py-8 text-xs text-slate-500">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              <ShieldCheck className="h-4 w-4 text-blue-600" />
              <span className="font-bold text-slate-900 font-mono">Fix It Preflight</span>
            </div>
